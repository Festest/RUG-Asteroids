...
public class SpaceshipViewModel extends GameObjectViewModel<Spaceship> {
	...
	public void draw(Graphics2D graphics2D, Point.Double location) {
		...
		this.drawUsername(spaceship, graphics2D, location);
	...
	private void drawMainBody(Spaceship spaceship, Graphics2D graphics2D, Point.Double location) {
		...
		graphics2D.setColor (spaceship.getSpaceshipColor());
	...
	/**
	 * Draw the username above the spaceship
	 * @param spaceship The spaceship whose name to draw.
	 * @param graphics2D The graphics object to use when drawing.
	 * @param location The location at which to draw the spaceship.
	 */
	private void drawUsername(Spaceship spaceship, Graphics2D graphics2D, Point.Double location) {
		int usernameLength = spaceship.getUsername().length();
		graphics2D.setColor(spaceship.getSpaceshipColor());
		graphics2D.drawString(spaceship.getUsername(),(int) location.getX() - usernameLength*5/2,(int) location.getY() - 35);
	}
}
